...
vi.mock('../EntryContentRenderer/EntryContentRenderer', () => ({
    default: ({ content }: { content: string }) => <div data-testid="entry-content">{content}</div>
}));
...
describe('ThoughtOfTheDay', () => {
    ...
    it('calls onClose on overlay click and Escape', async () => {
        (globalThis.fetch as Mock).mockResolvedValueOnce({ ok: true, json: async () => ({ randomEntry: null, onThisDay: {} }) });
        const onClose = vi.fn();
        ...
        const overlay = container.querySelector('.thought-of-day-overlay');
        if (overlay) {
