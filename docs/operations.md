...
The commands below describe the standalone profile in the `thoughty` namespace. On the BM Cluster production profile, use the `apps` namespace, inspect shared PostgreSQL in `infra`, and troubleshoot `ExternalSecret` status instead of Vault Agent files. See the [Server Deployment Guide](./server-deployment.md).
