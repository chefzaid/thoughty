...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name = 'entries' AND column_name = 'is_pinned') THEN
        ALTER TABLE entries ADD COLUMN is_pinned BOOLEAN DEFAULT false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
...
export class InitialSchema1784764800000 implements MigrationInterface {
    name = 'InitialSchema1784764800000';
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            DROP FUNCTION IF EXISTS set_ai_chat_histories_updated_at() CASCADE;
            DROP TABLE IF EXISTS cloud_sync_jobs CASCADE;
            DROP TABLE IF EXISTS ai_chat_histories CASCADE;
