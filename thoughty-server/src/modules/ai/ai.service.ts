...
export class AiService {
  ...
  /**
   * Weave a chapter's journal entries into flowing first-person prose, staying
   * strictly within the content of the entries. Long chapters are composed in
   * sequential parts so each request stays within a safe input budget.
   */
  async composeBookChapter(
    ...
    const model = await this.getModel(userId);
    const batches = this.batchEntriesByBudget(usableEntries, AiService.CHAPTER_INPUT_BUDGET);
    const parts: string[] = [];

    for (const batch of batches) {
      const previousEnding = parts.at(-1)?.slice(-400);
      parts.push(await this.requestChapterComposition(model, chapterTitle, batch, previousEnding));
    }
        ...
        messages: [
          {
            ...
            content: [
              ...
              'Weave them into flowing, readable prose: connect related thoughts, add brief transitions, smooth the grammar, and order ideas naturally.',
