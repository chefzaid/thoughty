...
const transcribeAttachment = vi.fn();

vi.mock("../../hooks/useApiServices", () => ({
  useApiServices: () => ({ attachmentsService: { transcribeAttachment } }),
}));

vi.mock("../AttachmentPreview/useAuthenticatedAttachmentUrl", () => ({
  useAuthenticatedAttachmentUrl: (storedFilename: string) =>
    `blob:${storedFilename}`,
}));
...
describe("AttachmentDisplay", () => {
  ...
  it("renders image attachments as clickable thumbnails", () => {
    ...
    render(
      <AttachmentDisplay attachments={attachments} theme="dark" t={mockT} />,
    );
  ...
  it("renders PDF attachments with an inline preview", () => {
    ...
    expect(
      screen.getByRole("link", { name: "downloadAttachment" }),
    ).toHaveAttribute("href", "blob:uuid-doc.pdf");
    expect(
      screen.getByRole("button", { name: "previewAttachment document.pdf" }),
    ).toBeInTheDocument();
  });
  ...
  it("renders audio attachments with an inline player", () => {
    ...
    expect(screen.getByText("downloadAttachment").closest("a")).toHaveAttribute(
      "href",
      "blob:uuid-voice.mp3",
    );
    expect(
      screen.getByRole("button", { name: "transcribeAudio" }),
    ).toBeInTheDocument();
  });

  it("transcribes audio and displays the returned text", async () => {
    const user = userEvent.setup();
    transcribeAttachment.mockResolvedValue({
      transcript: "A spoken journal thought.",
    ...
    await user.click(screen.getByRole("button", { name: "transcribeAudio" }));

    expect(transcribeAttachment).toHaveBeenCalledWith(3);
    expect(
      await screen.findByText("A spoken journal thought."),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "copyTranscript" }),
    ).toBeInTheDocument();
  });

  it("renders a stored transcript without requesting it again", () => {
    const attachments: Attachment[] = [
      {
        id: 3,
        original_filename: "voice-note.mp3",
    ...
    render(
      <AttachmentDisplay attachments={attachments} theme="light" t={mockT} />,
    );

    expect(screen.getByText("Persisted transcript")).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "transcribeAudio" }),
  ...
  it("opens lightbox when clicking on an image", async () => {
    ...
    const dialog = screen.getByRole("dialog", {
      name: "previewAttachment photo.png",
    });
  ...
  it("opens a large preview dialog for text attachments", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "previewAttachment notes.txt" }),
    );
