...
function AttachmentDisplay({ attachments, theme, t }: AttachmentDisplayProps) {
  const [activePreview, setActivePreview] =
    useState<ActivePreviewAttachment | null>(null);
  const { attachmentsService } = useApiServices();
  const isDark = theme !== "light";

  if (!attachments || attachments.length === 0) return null;

  const images = attachments.filter((a) => isImageAttachment(a.mimetype));
  const previewFiles = attachments.filter(
    (a) =>
      !isImageAttachment(a.mimetype) && hasInlineAttachmentPreview(a.mimetype),
  );
  const files = attachments.filter(
    (a) => !hasInlineAttachmentPreview(a.mimetype),
  );
            ...
            <StoredAttachmentResource key={file.id} attachment={file}>
              {(url) => (
                ...
                >
                  ...
                  <div
                    className={`flex min-h-[72px] items-center justify-center overflow-hidden rounded-md border px-2 py-2 ${isDark ? "border-gray-600 bg-gray-900/50" : "border-gray-200 bg-white"}`}
                  >
                    <AttachmentPreviewContent
                  ...
                  {isAudioAttachment(file.mimetype) && (
                    <AudioTranscript
                      attachment={file}
