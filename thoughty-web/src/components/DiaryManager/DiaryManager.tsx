...
interface DiaryManagerProps {
    ...
    readonly onCreateDiary: (data: DiaryFormData) => Promise<void>;
...
function DiaryManager({ diaries, onCreateDiary, onUpdateDiary, onDeleteDiary, onSetDefault, onBack, theme, t }: DiaryManagerProps): React.ReactElement {
    ...
    const handleEdit = (diary: Diary): void => {
    ...
    const handleSaveEdit = async (): Promise<void> => {
        if (!editName.trim() || !editingDiary) return;
        ...
        try {
            ...
            setError((err as Error).message || 'Failed to update diary');
        }
    };

    const handleDelete = async (diary: Diary): Promise<void> => {
        if (diary.is_default) {
            setError('Cannot delete the default diary');
            return;
        }
        if (globalThis.confirm(`${t('deleteDiary')}? ${t('deleteDiaryWarning')}`)) {
            try {
                await onDeleteDiary(diary.id);
            } catch (err) {
                setError((err as Error).message || 'Failed to delete diary');
            }
        }
    };

    const handleSetDefault = async (diary: Diary): Promise<void> => {
        try {
