...
vi.mock("./AiCredentialPanel", () => ({
  default: () => <div data-testid="ai-credential-panel" />,
}));
...
describe("AISection", () => {
  ...
  it("renders all AI settings fields", async () => {
    ...
    expect(
      screen.getByText("openRouterTaskModelsDescription"),
    ).toBeInTheDocument();
    ...
    expect(screen.getByText("openRouterPromptModel")).toBeInTheDocument();
  ...
  it("shows text input for model when models have not loaded", async () => {
    ...
    expect(screen.getAllByPlaceholderText("inheritDefaultModel")).toHaveLength(
      7,
    );
  });
  ...
  it("opens dropdown and shows models when trigger is clicked", async () => {
    ...
    expect(
      within(dropdownList).getByText("Claude 3.5 Sonnet"),
    ).toBeInTheDocument();
  ...
  it("selects a model when clicked in dropdown", async () => {
    ...
    fireEvent.click(
      within(dropdownList).getByRole("button", { name: /GPT-4o/ }),
    );
  ...
  it("displays selected model name when a model is configured", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /Claude 3.5 Sonnet/ }),
      ).toBeInTheDocument();
    });
  });
