...
const TASK_MODEL_FIELDS: ReadonlyArray<{
  name: keyof ProfileConfig;
  labelKey: string;
}> = [
  ...
  { name: "openRouterPromptModel", labelKey: "openRouterPromptModel" },
  { name: "openRouterBookModel", labelKey: "openRouterBookModel" },
];

function AISection({ localConfig, handleChange, t }: Readonly<AISectionProps>) {
  ...
  return (
    <div className="profile-section">
      ...
      <div className="section-content">
        <AiCredentialPanel t={t} onCredentialChanged={() => loadModels(true)} />
          ...
          <div className="ai-setting-field ai-setting-model" ref={dropdownRef}>
            ...
            {models.length > 0 ? (
              <div className="model-dropdown-container">
                ...
                {showDropdown && (
                  <div
                    className={`model-dropdown-panel ${localConfig.theme === "light" ? "light" : "dark"}`}
                  >
                    <input
                      type="text"
                      ...
                      {filteredModels.length === 0 ? (
                        ...
                        filteredModels.map((model) => (
                          <button
                            ...
                            onClick={() =>
                              handleSelectModel("openRouterModel", model.id)
                            }
                          >
                            <span className="model-dropdown-item-name">
                              {model.name}
                            </span>
                            <span className="model-dropdown-item-id">
                              {model.id}
                            </span>
                          </button>
                        ))
        ...
        <div className="ai-task-models">
          <div>
            <h4 className="ai-task-models-title">
              {t("openRouterTaskModels")}
            </h4>
            <p className="ai-task-models-description">
              {t("openRouterTaskModelsDescription")}
            </p>
          </div>
