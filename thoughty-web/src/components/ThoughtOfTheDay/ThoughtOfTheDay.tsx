...
function ThoughtOfTheDay({ isOpen, onClose, theme, t, diaryId, onNavigateToEntry }: ThoughtOfTheDayProps): React.ReactElement | null {
    ...
    const fetchHighlights = useCallback(async (): Promise<void> => {
        try {
            setLoading(true);
    ...
    const handleEntryClick = (entry: Entry): void => {
        if (onNavigateToEntry) {
            ...
            onNavigateToEntry(dateStr, entry.index || 1);
            onClose();
        }
    };
    ...
    const formatDate = (dateStr: string): string => {
        if (!dateStr) return '';
        if (dateStr.includes('T')) return dateStr.split('T')[0] ?? dateStr;
        return dateStr;
    };
                ...
                <div className="thought-of-day-body">
                    {loading ? (
                        ...
                        <div className="thought-of-day-content">
                            ...
                            {hasRandomEntry && highlights?.randomEntry && (
                                <div className="highlight-section random-thought">
                                    ...
                                    <div
                                        ...
                                        onKeyDown={(e: KeyboardEvent<HTMLDivElement>) => e.key === 'Enter' && handleEntryClick(highlights.randomEntry!)}
                                        role="button"
                                        tabIndex={0}
                                    >
                            ...
                            {hasOnThisDay && highlights?.onThisDay && (
                                                ...
                                                <div key={yearsAgo} className="year-group">
                                                    ...
                                                    {(highlights.onThisDay?.[yearsAgo] ?? []).map((entry) => (
                                                        <div
                                                            ...
                                                            onKeyDown={(e: KeyboardEvent<HTMLDivElement>) => e.key === 'Enter' && handleEntryClick(entry)}
                                                            role="button"
                                                            tabIndex={0}
                                                        >
                                                            <div className="entry-meta">
