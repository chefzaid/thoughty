...
describe("useAppState Hooks", () => {
  ...
  describe("useConfig", () => {
    ...
    it("applies high contrast class from config", async () => {
      ...
      await waitFor(() => {
        expect(document.body.classList.contains("high-contrast-mode")).toBe(
          true,
        );
      });
  ...
  describe("useDiaries", () => {
    ...
    it("propagates diary service failures", async () => {
      ...
      vi.mocked(createDiariesService).mockReturnValue({
        ...
        setDefaultDiary: vi
          .fn()
          .mockResolvedValue({ success: false, error: "Set default failed" }),
        reorderDiaries: vi.fn(),
      });
    ...
    it("refreshes entries after deleting the current diary", async () => {
      ...
      vi.mocked(createDiariesService).mockReturnValue({
        fetchDiaries: vi
          .fn()
          .mockResolvedValue([{ id: 2, name: "Other", is_default: true }]),
        createDiary: vi.fn(),
        updateDiary: vi.fn(),
  ...
  describe("useEntries", () => {
    ...
    it("updates filter and navigation state", () => {
      ...
      expect(result.current.sourceEntry).toEqual({
        id: 1,
        date: "2024-01-15",
        index: 1,
      });
    ...
    it("calculates page limit from config", () => {
      const numeric = renderHook(() =>
        useEntries(false, { entriesPerPage: 25 }, null),
      );
      const stringValue = renderHook(() =>
        useEntries(false, { entriesPerPage: "7" }, null),
      );
    ...
    it("runs diary-scoped meaning search and filters entries by ranked IDs", async () => {
      const fetchEntries = vi
        .fn()
        .mockResolvedValue({ entries: [], totalPages: 1, allTags: [] });
      ...
      const { createAiService, createEntriesService } =
    ...
    it("reorders the current day entries in local state before the API round-trip completes", async () => {
      ...
      await waitFor(() => {
        expect(
          result.current.groupedEntries["2024-01-15"]?.map((entry) => entry.id),
        ).toEqual([2, 1]);
